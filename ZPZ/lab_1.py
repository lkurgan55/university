...
class User():
    ...
    def menu(self, root):
        ...
        login_button = ttk.Button(root, text="change", command=lambda: change_pass(self.login, old_pass, password1, password2))
        login_button.pack(pady=10) 

        login_button = ttk.Button(root, text="Exit", command=lambda: exit(root))
        login_button.pack(pady=10)        
...
def change_pass(login, old_pass, password1, password2):
    ...
    if (login in USERS.keys()):
            if (USERS[login] == old_pass or USERS[login] == ""):
